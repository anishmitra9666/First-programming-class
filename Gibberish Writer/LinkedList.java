...
public class LinkedList<T> implements Iterable<T> {
  ...
  public ArrayList<T> toArrayList(){
    LLNode<T> firstNode = getFront();
    ArrayList<T> list = new ArrayList<T>();
    for (int i = 0; i < length(); i++){
      list.add(i, firstNode.getElement());
  ...
  public class ThisIterator implements LLIterator<T>{
    ...
    /** Create an iterator for the list starting at the inputted node
      ...
      */
    public ThisIterator(LLNode<T> front) {
      nodeptr = front;
      found = false;
    }
    ...
    public void addBefore(T element){
      if (isEmpty() ||  found == false){
        ...
        currentnode.setNext(new LLNode<T>(element, nodeptr));
      }
    }
    ...
    public void addAfter(T element){
      ...
      else if (nodeptr == null && found == true){
        addToEnd(element);
      }
      else{
  ...
  public LLIterator<T> iterator() {
    return new ThisIterator(getFront());
  } 
}